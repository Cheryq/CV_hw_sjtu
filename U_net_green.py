...
from extract import extract_img_data
from extract import get_label
...
def unet_train():
    ...
    for i in tqdm(range(n)):
        img = cv2.imread(path + input_name[i])
        resized_img = cv2.resize(img, (512, 512), fx=0, fy=0, interpolation=cv2.INTER_LINEAR)

        label = get_label(input_name[i],1)
        resized_label = cv2.resize(label, (512, 512), fx=0, fy=0, interpolation=cv2.INTER_LINEAR)

        X_train.append(resized_img)
        Y_train.append(resized_label)
    X_train = np.array(X_train)
    Y_train = np.array(Y_train)
    np.save('X_train_green', X_train)
    np.save('Y_train_green', Y_train)
...
def load_and_train(model_path, X_train, Y_train, i,epochs=1, batch_size=8):
    # Load saved model
    ...
    if not os.path.exists('model'):
        os.makedirs('model')
        # Open a text file for writing the loss values
    filename='model/training_loss'+str(i)+'.txt'
    with open(filename, 'w') as file:
        ...
        for epoch in range(epochs):
            ...
            model_name='model/unet_green'+str(i)+'.h5'
            # Save the updated model in the 'model' folder
            model.save(model_name)
            print('unet_green.h5 saved successfully in the model folder')

            # Write the loss value to the text file
            loss = history.history['loss'][0]
            file.write(f'{epoch+1},{loss:.6f}\n')
...
def unet_predict(unet, img_src_path):
    ...
    img_src = img_src.reshape(1, 512, 512, 3)  # shape为(1,512,512,3)
...
'''X_train=np.load('X_train_green.npy')
Y_train=np.load('Y_train_green.npy')
for i in range(10):
    if i==0:
        load_and_train('unet_green.h5', X_train,Y_train,i)
    else:
        load_and_train('model/unet_green'+str(i-1)+'.h5', X_train, Y_train, i)'''



'''model = models.load_model('model/unet_green25.h5')

img_src,img_mask=unet_predict(model, 'resources/images/difficult/3-2.jpg')
gray = cv2.cvtColor(img_src, cv2.COLOR_BGR2GRAY)
thresh = cv2.threshold(gray, 0, 255, cv2.THRESH_BINARY_INV | cv2.THRESH_OTSU)[1]

# 将白色车牌区域图像与二值化图像进行逻辑与操作
result = cv2.bitwise_and(thresh, thresh, mask=img_mask[:,:,0])
